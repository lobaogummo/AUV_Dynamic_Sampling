...
SCENARIO_NAME = "TEST_C4_NAZARE_30-10-2024_INST1_BASELINE"
CHOSEN_DAY_INDEX = 1
...
def main() -> None:
    ...
    source_nc = repo_root / "data" / "Test_C4" / "Priori_Nazare_30-10-2024_1" / "30-10-2024_predModel_1.nc"
    mask_out = repo_root / "data" / "Test_C4" / "Nazare_30-10-2024_1" / "mask.out"
    ...
    interface_nc = inputs_dir / "30-10-2024_predModel_1_planner_interface.nc"
    ...
    std_all = ds["STD"].values.astype(np.float32)  # day x LAT x LON
    if std_all.ndim != 3:
        raise RuntimeError(f"Unexpected STD dims: {std_all.shape}, expected (day, LAT, LON)")
    if not (0 <= CHOSEN_DAY_INDEX < std_all.shape[0]):
        raise RuntimeError(f"Invalid CHOSEN_DAY_INDEX={CHOSEN_DAY_INDEX} for STD shape {std_all.shape}")
    temperr = std_all[CHOSEN_DAY_INDEX].astype(np.float32, copy=False)
    ...
    # Keep NaNs over masked cells for the float layers.
    temperr = temperr.copy()
    tbath = tbath.copy()
    temperr[landt == 0] = np.nan
    ...
    out_ds["temperr"].attrs.update({"long_name": "temperature_uncertainty", "source_var": "STD", "day_index": CHOSEN_DAY_INDEX})
    ...
    manifest_payload = {
        ...
        "interface_rules": {
            "temperr": {"source": "STD", "day_index": CHOSEN_DAY_INDEX},
