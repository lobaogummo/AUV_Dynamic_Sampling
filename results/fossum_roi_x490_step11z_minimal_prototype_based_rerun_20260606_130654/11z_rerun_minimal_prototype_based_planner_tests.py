...
def load_step10f_temp_and_mask(step10f_dir: Path) -> tuple[np.ndarray, np.ndarray]:
...
def run_planner_case(
    ...
    attempted = any(
        ...
        for name in [
            ...
            "trajectory_waypoints.csv",
        ]
    )
    if skip_existing and attempted:
        routes = s11a.parse_routes_file(run_dir / "routes_file.txt")
        status = "REUSED" if routes else "REUSED_FAILED"
        return (
            {
                "run_id": run_id,
...
def plot_panels(outdir: Path, temp: np.ndarray, cases: pd.DataFrame, single_df: pd.DataFrame, multi_df: pd.DataFrame, route_points_by_run: dict[str, dict[str, list[tuple[int, int]]]]) -> None:
    ...
    fig, axes = plt.subplots(1, len(cases), figsize=(14, 4), squeeze=False)
    axes = axes.ravel()
    for ax, (_, case) in zip(axes, cases.iterrows()):
        ...
        for strategy, color in cmap.items():
            ...
            if pts:
                rr = [p[0] - ROI_ROW_MIN for p in pts]
                cc = [p[1] - ROI_COL_MIN for p in pts]
                ax.plot(cc, rr, color=color, lw=1, label=strategy)
        ax.set_title(case_id)
        ax.set_xlim(-0.5, temp[idx].shape[1] - 0.5)
        ax.set_ylim(-0.5, temp[idx].shape[0] - 0.5)
        ax.set_xticks([])
        ax.set_yticks([])
    ...
    for case_id, filename in [
        ...
        if case_id not in set(cases["case_id"].astype(str)):
            continue
...
def main() -> int:
    ...
    temp, mask = load_step10f_temp_and_mask(step10f_dir)
    ...
    checks = {
        ...
        "step10f_source": rel(step10f_dir),
