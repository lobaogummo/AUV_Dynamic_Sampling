...
DEFAULT_OUT_BASE = ROOT / "results" / "fossum_roi_x490_step04_sd_probe_patch40x24_dict4_20260511_211354"
DEFAULT_WORKING_SD_FRACTION = 0.30
DEFAULT_FRACTIONS = [DEFAULT_WORKING_SD_FRACTION]
DEFAULT_FIXED_PATCH_W = 40
DEFAULT_FIXED_PATCH_H = 24
...
def resolve_probe_input_paths() -> dict[str, Path]:
    paths = {
        "X_sst": _first_existing_file(
            [
                ROOT / "results" / "fossum_roi_x490_step00_dataset_20260509_232915" / "X_surface_370_roi_x490.npy",
            ]
        ),
        "X_norm": _first_existing_file(
            [
                ROOT / "results" / "fossum_roi_x490_step00_dataset_20260509_232915" / "X_surface_370_roi_x490_norm.npy",
            ]
        ),
        "mask": _first_existing_file(
            [
                ROOT / "results" / "fossum_roi_x490_step00_dataset_20260509_232915" / "mask_common_roi_x490.npy",
            ]
        ),
        "png_dir": _first_existing_dir(
            [
                ROOT / "results" / "fossum_roi_x490_step00_dataset_20260509_232915" / "legacy_named_normalized_pngs",
                ROOT / "results" / "fossum_roi_x490_step00_dataset_20260509_232915" / "normalized_pngs",
            ]
        ),
    }
...
def build_png_map_from_dir(png_dir: Path) -> dict[int, Path]:
    ...
    if files:
        ...
        files = sorted(png_dir.glob("*_X_surface_370_roi_x490_norm.png"))
        for p in files:
            try:
                z = int(p.name.split("_", 1)[0])
            except Exception:
                continue
            out[z] = p
    if not out:
