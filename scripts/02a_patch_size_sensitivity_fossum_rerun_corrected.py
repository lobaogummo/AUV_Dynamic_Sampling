"""Complete rerun of Fossum patch-size sensitivity with corrected seed variability."""
...
DEFAULT_OUT_BASE = ROOT / "results" / "fossum" / "patch_size_sensitivity_fossum_rerun_corrected"
DEFAULT_DOC = ROOT / "docs" / "PATCH_SIZE_SENSITIVITY_FOSSUM_RERUN_CORRECTED.md"
...
def log(msg: str) -> None:
    print(f"[rerun-corrected] {msg}")


def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Fossum patch-size sensitivity rerun (corrected stochastic stream).")
    p.add_argument("--out-base", type=Path, default=DEFAULT_OUT_BASE)
    p.add_argument("--doc-path", type=Path, default=DEFAULT_DOC)
    p.add_argument("--patch-sizes", nargs="*", default=None, help='Subset ex: "32x20 48x32"')
    p.add_argument("--seeds", nargs="*", type=int, default=None)
    p.add_argument("--no-resume", action="store_true")
    return p.parse_args()


def parse_patch_sizes(values: Sequence[str] | None) -> List[Tuple[int, int]]:
    if not values:
        return DEFAULT_PATCH_SIZES.copy()
    out: List[Tuple[int, int]] = []
    rx = re.compile(r"^\s*(\d+)\s*[xX]\s*(\d+)\s*$")
    for v in values:
        m = rx.match(v)
        if not m:
            raise ValueError(f"Invalid patch size token '{v}'. Use WxH.")
...
def train_dictionary_seeded_stream(X: np.ndarray, patch_h: int, patch_w: int, seed: int) -> MiniBatchDictionaryLearning:
...
def save_member_contact_sheets_from_png_source(
    ...
    for ci, idx in enumerate(class_indices, start=1):
        ...
        title = f"Class {ci:02d} members (n={len(idx)}, shown={len(panel_pngs)}) [{patch_w}x{patch_h}] seed={seed}"
...
def run_single(
    ...
    model = train_dictionary_seeded_stream(X_norm, patch_h=patch_h, patch_w=patch_w, seed=seed)
    features = encode_images(X_norm, model=model, patch_h=patch_h, patch_w=patch_w)
...
def label_from_pair(pw: int, ph: int) -> str:
    return f"{int(pw)}x{int(ph)}"
...
def plot_boxplot_icv(runs_df: pd.DataFrame, out_plots: Path) -> None:
    ...
    for (pw, ph), sub in ordered.groupby(["patch_w", "patch_h"]):
        labels.append(label_from_pair(int(pw), int(ph)))
...
def plot_boxplot_zoom_centered(runs_df: pd.DataFrame, out_plots: Path) -> None:
    ...
    ax.set_ylim(-lim, +lim)
    ax.set_title("Fig 6a zoom: centered mean ICV by patch size")
    ax.set_xlabel("Patch size (w x h)")
    ax.set_ylabel("mean_icv - patch_mean_icv")
    ax.grid(True, linestyle="--", alpha=0.3)
    plt.setp(ax.get_xticklabels(), rotation=30, ha="right")
    fig.tight_layout()
    fig.savefig(out_plots / "fig6a_icv_boxplot_patchsize_zoom.png", dpi=170)
    plt.close(fig)


def build_variance_table(runs_df: pd.DataFrame) -> pd.DataFrame:
    rows = []
    for (pw, ph), sub in runs_df.groupby(["patch_w", "patch_h"]):
        ...
        rows.append(
            {
                "patch_w": int(pw),
                "patch_h": int(ph),
                "patch_label": label_from_pair(int(pw), int(ph)),
                "mean_icv_min": mn,
                "mean_icv_max": mx,
...
def plot_seed_points(runs_df: pd.DataFrame, out_plots: Path) -> None:
    fig, ax = plt.subplots(figsize=(11.5, 5.2))
    ordered = runs_df.sort_values(["patch_w", "patch_h", "seed"])
    groups = list(ordered.groupby(["patch_w", "patch_h"]))
    x = np.arange(len(groups))
    labels = []
    for i, ((pw, ph), sub) in enumerate(groups):
        labels.append(label_from_pair(int(pw), int(ph)))
        vals = sub["mean_icv"].to_numpy(dtype=float)
    ...
    ax.set_title("ICV seed points by patch size")
    ax.set_xlabel("Patch size (w x h)")
    ax.set_ylabel("Mean ICV (SST/original space)")
    ax.grid(True, linestyle="--", alpha=0.3)
    fig.tight_layout()
    fig.savefig(out_plots / "icv_seed_points_by_patchsize.png", dpi=170)
    plt.close(fig)
...
def plot_summary_metric(summary_df: pd.DataFrame, y_col: str, yerr_col: str | None, out_path: Path, title: str, ylabel: str) -> None:
    labels = [label_from_pair(int(r["patch_w"]), int(r["patch_h"])) for _, r in summary_df.iterrows()]
...
def write_report(
    ...
    md = [
        ...
        "",
        "## Configuracao",
        f"- xds={N_DICT}, N_CLASSES={N_CLASSES}, stride=1, todos os patches possiveis",
        f"- patch sizes: {', '.join([f'({w},{h})' for w, h in patch_sizes])}",
        f"- seeds: {', '.join(str(s) for s in seeds)}",
        "",
        "## Variabilidade entre seeds",
        f"- patch sizes com mean_icv_std > 0: {nonzero_spread} / {len(summary_df)}",
        "",
        "## Summary",
        make_md_table(
            summary_df[
        ...
        make_md_table(
            top[["patch_w", "patch_h", "balanced_score", "mean_icv_mean", "mean_icv_std", "min_class_size_min", "runtime_mean_seconds"]],
        ...
        f"- prototipos: `{(out_base / 'prototypes_wXX_hYY_seedSS').relative_to(ROOT).as_posix()}`",
        f"- paineis de classe: `{(out_base / 'class_members_wXX_hYY_seedSS').relative_to(ROOT).as_posix()}`",
    ]
...
def main() -> None:
    args = parse_args()
    patch_sizes = parse_patch_sizes(args.patch_sizes)
    ...
    done = {(r.patch_w, r.patch_h, r.seed) for r in runs if r.notes == "ok"}
    ...
    plot_summary_metric(
        ...
        out_path=out_plots / "icv_mean_vs_patchsize.png",
        title="Mean ICV (SST/original) vs Patch Size (error bars: run std)",
    ...
    plot_summary_metric(
        ...
        out_path=out_plots / "icv_std_vs_patchsize.png",
        title="Std(ICV across classes, SST/original) vs Patch Size",
    ...
    plot_summary_metric(
        ...
        yerr_col=None,
        out_path=out_plots / "min_class_size_vs_patchsize.png",
    ...
    plot_summary_metric(
        ...
        out_path=out_plots / "runtime_vs_patchsize.png",
        title="Runtime vs Patch Size",
