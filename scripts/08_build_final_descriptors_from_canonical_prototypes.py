...
def old_descriptor_logic_audit(output_dir: Path) -> dict[str, Any]:
    ...
    components = [
        ...
        },
        {
            ...
            "logic": "multi_regime: Otsu segmentation + interface boundary + distance-to-boundary + 0.65 gradient/0.35 proximity boundary_score; single_gradient: continuous gradient boundary proxy capped at 0.35; homogeneous: single region with zero boundary.",
            "decision": "REUSE_WITH_ADAPTATION",
    ...
    answers = {
        ...
        "6_calculation_summary": {
            ...
            "boundary_score": "multi-regime 0.65*gradient_norm + 0.35*boundary_proximity; single-gradient capped gradient proxy; homogeneous zero.",
            ...
            "segmentation": "label-driven: homogeneous/single-gradient single region; multi-regime Otsu low/high regions.",
        ...
        "8_thresholds": [
            ...
            "single_gradient boundary proxy max_score=0.35",
        ],
...
def save_multimap_panel(
    ...
    for i in range(n):
        for j, label in enumerate(labels):
            ...
            arr = arrays[j][i]
            if label == "prototype":
                im = ax.imshow(arr, origin="lower", cmap=cmaps[j], aspect="auto")
...
def main() -> None:
    ...
    for i in range(n_classes):
        ...
        regime = normalize_regime_label(cv_row.get("regime_label", "unknown"))
        if regime not in {"homogeneous", "single_gradient", "multi_regime"}:
            regime = "multi_regime" if class_id in {1, 6} else "single_gradient"
        ...
        if regime == "multi_regime":
            ...
            boundary_mask = high_grad_mask if regime == "single_gradient" else np.zeros(proto.shape, dtype=bool)
            boundary_threshold = high_grad_thr if regime == "single_gradient" else float("nan")
            boundary_method = "gradient_p90_proxy" if regime == "single_gradient" else "label_driven_zero"
