...
plt.imshow(temperr2d_masked, interpolation='none',cmap='viridis',origin='lower')  
plt.grid()
plt.colorbar()
plt.title('Temperature error - HOPS Area - land mask')
plt.show()


#Operation area with all masks
plt.imshow(temperr2d_op, interpolation='none',cmap='viridis',origin='lower')  
plt.grid()
plt.colorbar()
plt.title('Temperature error - Operational Area - all masks')
plt.show()
...
plt.scatter(*zip(*uncertain_points_for_scatter), s=5, c='black', marker='o')  
...
plt.colorbar()
uncertain_points_for_scatter = [(y, x)  for x, y in uncertain_points]
plt.scatter(*zip(*uncertain_points_for_scatter), s=5, c='grey', marker='o', label = "Total VRP points: "+str(len(uncertain_points))+"\n___________")  #RICORDA LO SCATTER VUOLE PRIMA Y poi X
for i in range(len(vrp_routes_points_wt_clean)):
    single_vrp_route_scatter = [(y, x)  for x, y in vrp_routes_points_wt_clean[i]]
    plt.plot(*zip(*single_vrp_route_scatter), label = "Visited points: "+str(len(vrp_routes_points_wt[i])-2)+"\nWaypoints: "+str(len(vrp_routes_points_wt_clean[i])-2)+"\n___________")
    plt.scatter(*zip(*single_vrp_route_scatter), s=5, c='black', marker='o')
    plt.scatter(single_vrp_route_scatter[0][0], single_vrp_route_scatter[0][1], s=80, c='black', marker='*')    #first point
    plt.scatter(single_vrp_route_scatter[1][0], single_vrp_route_scatter[1][1], s=30, c='purple', marker='o')    #second point
plt.legend(bbox_to_anchor = (1.25, 0.6), loc='upper left')
#save plot
date_time = datetime.datetime.utcnow()
file_name = "plots/"+str(date_time)+"_wt.png"
plt.title('PC-VRP Solution')
